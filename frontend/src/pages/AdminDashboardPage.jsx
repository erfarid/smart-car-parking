...
// Default empty zone (new zone create karne ke liye template)
const emptyZone = {
  zone_id: "",
  zone_name: "",
...
export default function ZoneConfigPage() {
  ...
  // Form state (editing ya new zone dono handle karega)
  const [editing, setEditing] = useState(null);
  const [form, setForm] = useState({ ...emptyZone });
  const [showForm, setShowForm] = useState(false);

  // Page load hote hi zones fetch karo
  useEffect(() => {
    loadZones();
  }, []);
  ...
  // Submit -> create ya update decide karega
  async function handleSubmit(e) {
    ...
    try {
      if (editing) {
        ...
        setMsg({
          type: "success",
          text: `Zone ${editing} updated successfully.`,
        });
      } else {
        await ApiClient.createZone(form);
        setMsg({
          type: "success",
          text: `Zone ${form.zone_id} created successfully.`,
        });
      }
      setShowForm(false);
      setEditing(null);
  ...
  return (
    <div className="page">
      ...
      {/* Zones table (list + edit/delete actions) */}
      <div className="card">
